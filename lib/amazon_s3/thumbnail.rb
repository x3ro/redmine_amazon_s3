module AmazonS3
  class Thumbnail
    ...
    def self.get(source, target, size, update_thumb = false)
      ...
      Connection.object_url(target, target_folder)
    end
    ...
    def self.generate(source, target, size, target_folder)
      ...
      url = Connection.object_url(source)
      open(url, 'rb') { |f| img = img.from_blob(f.read) }
      img = img.strip!
      img = img.resize_to_fit(size)

      Connection.put(target, File.basename(target), img.to_blob, img.mime_type, target_folder)
    end
  end
end
